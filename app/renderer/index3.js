...
// ..... open the image in file explorer !!!
...
$('#donate-button-id').on('click', e => {
  console.log("clicked");
  shell.openExternal("https://paypal.me/dipankarpal5050");
});
...
function textToQR(txt) {
  ...
  request("https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=" + txt)
    ...
    if (global.globalToken == "") {
      ...
      if (event.dataTransfer.files.length == 1) {
        ...
        // If not valid video file, then show error and bring back to home page
        {
          ...
          $(".search-box-div").fadeIn("fast");
          $("#drag-caption-id").text("Only jpg and png files are supported");
...
function loadSearchPage() {
  ...
  $(".main-window").css("background", "linear-gradient(to bottom,  #ADD372 0%, #8EC89F 85%,#77C0C0 100%)");
  $("#searching-sub-for-id").text('Loading');
  $("#loading-id").hide();
...
function showQueryFailurePage() {
  ...
  $(".fail-to-find-text").text('oops! something went wrong');
  $(".fail-to-find-text").fadeIn("slow");
}
